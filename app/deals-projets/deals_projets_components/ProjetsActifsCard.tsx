  ...
  return (
    <div className="rounded-2xl md:rounded-xl p-6 md:p-5 h-full flex justify-between overflow-hidden transition-all duration-300 ease-out
      ...
      <div className="flex flex-col gap-3 md:gap-1.5 flex-1 min-w-0">
        <p className="text-gray-500 md:text-[#ED8600] text-xs md:text-lg font-medium md:font-bold uppercase md:normal-case tracking-widest md:tracking-normal">Projets actifs</p>
        <p className="text-gray-800 md:text-gray-500 text-3xl md:text-[clamp(28px,3vw,40px)] font-bold md:font-semibold tracking-tight tabular-nums">{projetsActifs}</p>
        <div className="flex flex-wrap gap-2 items-center mt-1 md:mt-2">
          <span className="inline-flex px-2.5 py-1 rounded-lg md:rounded-full text-sm font-medium text-emerald-700 bg-emerald-500/10 md:bg-green-700 md:text-white">Actifs</span>
          <span className="text-gray-400 md:text-gray-500 text-xs md:text-sm">en cours</span>
        </div>
      </div>
