  ...
  return (
    <div className="rounded-2xl md:rounded-xl p-6 md:p-5 h-full flex justify-between overflow-hidden transition-all duration-300 ease-out
      ...
      <div className="flex flex-col gap-3 md:gap-1.5 flex-1 min-w-0">
        <p className="text-gray-500 md:text-[#ED8600] text-xs md:text-lg font-medium md:font-bold uppercase md:normal-case tracking-widest md:tracking-normal">Total clients</p>
        <p className="text-gray-800 md:text-gray-500 text-3xl md:text-[clamp(28px,3vw,40px)] font-bold md:font-semibold tracking-tight tabular-nums">{totalClients}</p>
        <p className="text-gray-400 md:text-gray-500 text-xs md:text-sm mt-1 md:mt-2">Depuis toujours</p>
      </div>
