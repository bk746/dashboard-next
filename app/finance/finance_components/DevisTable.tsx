          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-300 dark:border-gray-700">
                <th className="text-left p-4 text-gray-500 dark:text-gray-400 text-sm font-semibold">Numéro</th>
                ...
                <th className="text-left p-4 text-gray-500 dark:text-gray-400 text-sm font-semibold">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredDevis.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-gray-500 dark:text-gray-400">Aucun devis trouvé</td>
                </tr>
              ) : (
                filteredDevis.map((d) => (
                  <tr key={d.id} className="border-b border-gray-300 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-800">
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm">{d.numeroDevis}</td>
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm">{d.entreprise}</td>
                    <td className="p-4">
                      <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${getStatutBadgeColor(d.statut)}`}>
                        {d.statut}
                      </span>
                    </td>
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm">{d.date}</td>
                    <td className="p-4 text-gray-500 dark:text-gray-400 text-sm">{d.prix.toLocaleString("fr-FR")} €</td>
                    <td className="p-4">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#ED8600] dark:bg-blue-800 rounded-lg text-white text-sm hover:opacity-90"
                        >
                          <FaFileInvoice className="text-xs" /> Créer la facture
                        </button>
                        <button onClick={() => onEdit(d)} className="text-gray-500 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2">
