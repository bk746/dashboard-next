  ...
  return (
    <div className="rounded-2xl md:rounded-xl p-6 md:p-5 h-full flex justify-between overflow-hidden transition-all duration-300 ease-out
      ...
      <div className="flex flex-col gap-3 md:gap-1.5 flex-1 min-w-0">
        <p className="text-gray-500 md:text-[#ED8600] text-xs md:text-lg font-medium md:font-bold uppercase md:normal-case tracking-widest md:tracking-normal">Dépenses</p>
        ...
        <div className="flex flex-col gap-1 mt-1 md:mt-2">
          {totalRecurrent > 0 && (
            <div className="flex items-center gap-2 text-gray-500 text-xs md:text-sm">
              <span className="px-2 py-0.5 rounded-lg md:rounded bg-amber-100 md:bg-amber-200 text-amber-800 md:text-amber-900 text-xs">Récurrent / mois</span>
              <span>{totalRecurrent.toLocaleString("fr-FR")} €</span>
            </div>
          )}
          {totalOccasionnel > 0 && (
            <div className="flex items-center gap-2 text-gray-500 text-xs md:text-sm">
              <span className="px-2 py-0.5 rounded-lg md:rounded bg-gray-100 md:bg-gray-300 text-gray-600 md:text-gray-700 text-xs">Occasionnel</span>
      ...
      <div className="hidden sm:flex flex-shrink-0 items-start">
        <FaWallet className="h-10 w-10 sm:h-11 sm:w-11 text-[#ED8600]/80" />
      </div>
    </div>
  );
}
